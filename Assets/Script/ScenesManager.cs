...
public class ScenesManager : MonoBehaviour
{
    ...
    {
        if (lastLevel == "level1" || lastLevel == "level2")
        {
            GameStats gameStats = new GameStats();
            gameStats.liveLeft = GameManager.playerLives;
            ...
            //obtener datos JSON de origen
            string datosJson = System.IO.File.ReadAllText(Application.persistentDataPath + "/GameStatsSaved.json");
            //almacenar en clase tipo
            GameStats datosPartida = new GameStats();
            datosPartida = JsonUtility.FromJson<GameStats>(datosJson);
            //mostrar consola
            Debug.Log("datos leidos: " + datosJson);
        }
    }
}
